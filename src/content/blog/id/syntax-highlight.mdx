...
authors:
    - id/inurhuda00
desc: 'highlighter yang memberikan kode yang indah'
topics: ['docs']
tags:
    - rehype
    - pretty
...
[Rehype Pretty Code] (https://github.com/atomiks/rehype-pretty-code) adalah plugin Rehype
yang didukung oleh sintaks [Shiki] (https://github.com/shikijs/shiki)
highlighter yang menyediakan blok kode cantik untuk Markdown atau MDX. Ini cepat
karena menghindari penyorotan sintaks runtime dengan mengeksekusi pada saat build-time, dan
bekerja dengan fitur-fitur baru seperti React Server Components.

# Contoh markdown

Blok kode berikut ini akan disorot sebagai JavaScript,
dibungkus dalam bingkai editor kode dengan nama file "example.js",
dan rentang baris 2-4 akan ditandai:

```js title="Contoh.js" {2-4}
// Contoh function
function add(a, b) {
	return a + b
}
...
Memanfaatkan keakuratan mesin penyorotan sintaksis VS Code dan popularitas
ekosistem temanya - gunakan tema VS Code apa pun yang Anda inginkan!
...
// ESM
const options = {
	theme: JSON.parse(readFileSync(new URL('./themes/theme.json', import.meta.url))),
}
```
